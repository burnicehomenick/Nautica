...
var PORTS = [443, 80];
var PROTOCOLS = [reverse("najort"), reverse("sselv"), reverse("ss")];
var KV_PROXY_URL = "https://raw.githubusercontent.com/burnicehomenick/Nautica/refs/heads/main/kvProxyList.json";
...
__name(getKVProxyList, "getKVProxyList");
...
function getAllConfig(request, hostName, proxyList, page = 0) {
  ...
  try {
    ...
    for (let i = startIndex; i < startIndex + PROXY_PER_PAGE; i++) {
      ...
      if (!proxy) break;
      const { proxyIP: proxyIP2, proxyPort, country, org } = proxy;
      uri.searchParams.set("path", `/${proxyIP2}-${proxyPort}`);
      const proxies = [];
        ...
        for (const protocol of PROTOCOLS) {
          if (protocol === "ss") {
            ...
            uri.searchParams.set(
              "plugin",
              `v2ray-plugin${port == 80 ? "" : ";tls"};mux=0;mode=websocket;path=/${proxyIP2}-${proxyPort};host=${hostName}`
            );
          } else {
            uri.username = uuid;
    ...
    return `An error occurred while generating the ${reverse("SSELV")} configurations. ${error3}`;
  }
}
  ...
  async fetch(request, env2, ctx) {
    try {
      ...
      if (url.pathname.startsWith("/sub")) {
        ...
        const proxyBankUrl = url.searchParams.get("proxy-list") || env2.PROXY_BANK_URL;
        ...
        if (apiPath.startsWith("/domains")) {
          ...
          if (wildcardApiPath == "/get") {
            ...
            const register = await cloudflareApi.registerDomain(domain2);
          ...
          const proxyBankUrl = url.searchParams.get("proxy-list") || env2.PROXY_BANK_URL;
          const proxyList = await getProxyList(proxyBankUrl).then((proxies) => {
            if (filterCC.length) {
      ...
      const targetReverseProxy = env2.REVERSE_PROXY_TARGET || "example.com";
  ...
  readableWebSocketStream.pipeTo(
    new WritableStream({
      async write(chunk, controller) {
        if (isDNS) {
          return handleUDPOutbound(DNS_SERVER_ADDRESS, DNS_SERVER_PORT, chunk, webSocket, null, log3);
        }
...
async function protocolSniffer(buffer) {
  if (buffer.byteLength >= 62) {
    ...
    if (najortDelimiter[0] === 13 && najortDelimiter[1] === 10) {
      if (najortDelimiter[2] === 1 || najortDelimiter[2] === 3 || najortDelimiter[2] === 127) {
        if (najortDelimiter[3] === 1 || najortDelimiter[3] === 3 || najortDelimiter[3] === 4) {
...
async function handleUDPOutbound(targetAddress, targetPort, udpChunk, webSocket, responseHeader, log3) {
  try {
    ...
    log3(`Connected to ${targetAddress}:${targetPort}`);
...
__name(parseSsHeader, "parseSsHeader");
...
__name(parseSselvHeader, "parseSselvHeader");
...
__name(parseNajortHeader, "parseNajortHeader");
...
var CloudflareApi = class {
  ...
  async registerDomain(domain2) {
    ...
    if (!domain2.endsWith(rootDomain)) return 400;
    if (registeredDomains.includes(domain2)) return 409;
    try {
      const domainTest = await fetch(`https://${domain2.replaceAll("." + APP_DOMAIN, "")}`);
      ...
      if (badWordsListRes.status == 200) {
        ...
        for (const badWord of badWordsList) {
          if (domain2.includes(badWord.toLowerCase())) {
            return 403;
          }
        }
      } else {
...
var Document = class {
  ...
  setTitle(title2) {
    this.html = this.html.replaceAll("PLACEHOLDER_JUDUL", title2);
  }
  addInfo(text) {
    text = `<span>${text}</span>`;
